...
log = get_logger("routes.wellness")
...
def _compute_breakdown(entry: dict) -> dict[str, int]:
    ...
    hydration_score = round(min(15.0, (min(water_glasses, _HYDRATION_TARGET_GLASSES) / _HYDRATION_TARGET_GLASSES) * 15))
...
async def get_wellness_score(athlete_id: str, _: dict = Depends(require_athlete_or_admin("athlete_id"))):
    ...
    cache_key = f"wellness:{athlete_id}"
