...
patch("routes.nutrition.FOODS_CACHE", MOCK_FOODS).start()
...
def test_log_meal_success():
    """Log a single item meal and verify response shape and computed macros."""
    ...
    # Oats: 389 cal/100g → 200g = 778 cal
    assert data["total"]["calories"] == pytest.approx(778.0, rel=1e-2)


def test_log_meal_two_items_total_is_sum():
    """Total macros must equal the sum of both items individually."""
    resp = client.post(f"/athlete/{ATHLETE_ID}/meals", json={
        "date":  VALID_DATE,
        "slot":  "lunch",
        "items": [
            {"food_id": "f_003", "qty_g": 150},  # Chicken: 165 * 1.5 = 247.5
            {"food_id": "f_004", "qty_g": 200},  # Rice:    216 * 2.0 = 432.0
        ],
    })
    assert resp.status_code == 201, resp.json()
...
def test_log_meal_overwrites_same_slot():
    """Posting to the same slot twice should overwrite, not accumulate."""
...
def test_get_meals_returns_grouped_by_slot():
    """After logging breakfast and lunch, GET should return both grouped correctly."""
    client.post(f"/athlete/{ATHLETE_ID}/meals", json={
